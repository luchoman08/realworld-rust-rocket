...
use crate::schema::{articles, favorites, follows, users};
...
pub fn create(
    ...
    diesel::insert_into(articles::table)
        ...
        .attach(author.into_profile(false), false)
}
...
pub fn find(conn: &PgConnection, params: &FindArticles, user_id: Option<i32>) -> Vec<ArticleJson> {
    ...
    query
        ...
        .map(|(article, author, favorited)| {
            article.attach(into_profile_for(conn, author, user_id), favorited)
        })
        .collect()
}
...
pub fn feed(conn: &PgConnection, params: &FeedArticles, user_id: i32) -> Vec<ArticleJson> {
    articles::table
        ...
        .into_iter()
        .map(|(article, author, favorited)| {
            article.attach(into_profile_for(conn, author, Some(user_id)), favorited)
        })
        .collect()
}
